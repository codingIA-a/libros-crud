...
class Author:
    ...
    #crear método para añadir un libro a favoritos
    @classmethod
    def add_favorite(cls, data):
        query = 'insert into favorites (fk_author_id, fk_book_id) values (%(author_id)s, %(book_id)s);'
        results = connectToMySQL('libros').query_db(query, data)
        return results
    #autores que no han añadido el libro como favorito
    @classmethod
    def unfavorited_authors(cls, data):
        query = 'select * from authors where authors.id not in (select fk_author_id from favorites where fk_book_id = %(id)s);'
        results = connectToMySQL('libros').query_db(query, data)
        authors = []
        for row in results:
